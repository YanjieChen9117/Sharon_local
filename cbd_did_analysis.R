...
# 加载必要的库
# 检查并安装缺失的包
required_packages <- c("dplyr", "lubridate", "broom", "sandwich", "lmtest")
...
run_did_analysis <- function(data, outcome_var, outcome_name) {
  ...
  # 使用稳健标准误（Heteroskedasticity-robust standard errors）
  model_robust <- coeftest(model, vcov = vcovHC(model, type = "HC3"))
  
  # 提取treatment_post系数（政策效应）
  treatment_effect <- model_robust["treatment_post", "Estimate"]
  treatment_se <- model_robust["treatment_post", "Std. Error"]
  treatment_pvalue <- model_robust["treatment_post", "Pr(>|t|)"]
...
for (var in c("outflow_trips", "inflow_trips", "outflow_avg_speed", "inflow_avg_speed")) {
  ...
  cat(sprintf("\n%s:\n", var))
  cat(sprintf("  Treatment group - Pre: %.4f, Post: %.4f, Change: %.4f\n", 
              treatment_pre, treatment_post, treatment_change))
  cat(sprintf("  Control group - Pre: %.4f, Post: %.4f, Change: %.4f\n", 
